...
async def send_user_echo(message: Message):
    await message.reply_user_message(message.user_message.file_id)

async def send_poll_echo(message:Message):
    await message.reply_poll(message.poll.file_id)
...
dp.message.register(send_user_echo, F.user)
